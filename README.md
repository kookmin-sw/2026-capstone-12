...
2. Lobby에서 **방 만들기** 또는 **방 참가하기** (4자리 방 코드 공유)
3. RoomScene에서 **역할 선택** (Shooter / Supporter) 후 Ready
4. 두 플레이어 모두 Ready 완료 시 전투 시작
...
**목표**: 적을 처치하고 SpawnCore를 공격하며, CommandTower가 파괴되지 않도록 전투를 이어간다.
...
**목표**: 방어 구조물과 지원 아이템으로 Shooter와 CommandTower를 지원한다. 전장 전체를 확인하며 적의 이동 경로, Shooter의 상태, 자원 상황을 함께 고려해야 한다.
...
| 종류 | 분류 | 비용 | 기능 |
|------|------|------|------|
| Turret | 방어 구조물 | 50G | 범위 내 적을 자동으로 공격 |
| SlowTower | 방어 구조물 | 50G | 범위 내 적 이동 속도 감소 |
| Barrier | 방어 구조물 | 10G | 적 이동 경로 차단 |
| LightPylon | 지원 구조물 | 100G | 반경 16m 영구 정화 구역 제공 |
| HealthPack | 지원 아이템 | - | Shooter가 접근하면 체력 회복 |
| AmmoPack | 지원 아이템 | - | Shooter가 접근하면 탄약 보충 |
| PurificationBeacon | 지원 아이템 | - | 일정 시간 동안 임시 정화 구역 제공 |

> 건설 방법: 구조물 선택 → Ghost Preview 확인 → Q/E로 회전 → 좌클릭으로 배치  
> 구조물 수리/판매: 설치된 구조물 클릭 → 팝업에서 Repair / Sell 선택

---

### 정화 시스템

Shooter는 **정화 게이지**에 따라 주변에 정화 구역을 형성합니다.  
정화 구역 밖에서는 시야가 제한되고 어둠 노출 피해를 받습니다.
...
LightPylon, PurificationBeacon, SpawnCore 파괴로도 정화 구역을 확장할 수 있습니다.
...
| 조건 | 내용 |
|------|------|
| 승리 | 모든 SpawnCore를 파괴하면 Victory |
| 패배 | CommandTower가 파괴되면 GameOver |
